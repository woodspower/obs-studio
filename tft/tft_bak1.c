...
/* max 5hours*10frame/second */
#define TF_TAG_BATCH_NUM_MAX 5*3600*10

struct obs_source {
	char *name;
};

enum tf_tag_batch_item_pose_type {
	TF_TAG_POSE_TYPE_1,
	TF_TAG_POSE_TYPE_2
};

struct tf_tag_batch_item {
	char *name;
	struct obs_source *source;
	struct tf_tag_batch_item *prev;
	struct tf_tag_batch_item *next;
	enum tf_tag_batch_item_pose_type pose;
	unsigned int pose;
	unsigned int xmin;
	unsigned int ymin;
	unsigned int xmax;
...
struct tf_tag_batch {
	volatile long        ref;
	struct tf_tag_batch_item *items;
};



static inline void tf_tag_load(obs_data_t *data, json_t *jobj)
{
	const char *item_key;
	json_t *jitem;
